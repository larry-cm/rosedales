...
export default function Carousel({arrayL,children:slides}:{arrayL:number,children?:React.ReactElement[]}){

  const [position,setPosition] = useState(0)

  const nextPosition= () =>setPosition(p => slides && p === arrayL - 1 ? 0 : p+1 )
  const prevPosition= () =>setPosition(p => slides && p === 0 ? arrayL - 1 : p-1)
console.log(slides);

  return (
    <div className="overflow-hidden relative bg-green-500">
      <div className="flex transition duration-300 ease-in-out"
        style={{
